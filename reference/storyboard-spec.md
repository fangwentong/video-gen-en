...
- **Scene**: A narrative unit with relatively stable semantics, visuals, and space-time, typically lasting 10-30 seconds
- **Shot**: The smallest video generation unit, lasting 2-5 seconds
...
|-------|---------|-------------------|---------|
| **Element ID** | Technical ID for JSON references, character identification in prompts | `Element_` + English name/Pinyin | `Element_Chuyue`, `Element_Xiaomei` |
| **Display Name** | Display name for user interaction | Character name | `Luna`, `May` |
...
- **Note**: Phase 1 only handles user-uploaded reference images; `reference_images` for unuploaded ones remain empty, to be filled by Phase 2
...
**Phase 3: Storyboard Design (LLM Auto-Generate)**
- LLM generates shots based on `elements.characters`
...
- **Select generation mode based on project type**:
  - Fiction/Short drama, MV shorts → **All shots require storyboard frames** → `reference2video` (Omni) or `img2video` (fallback)
  - Vlog/Documentary, Commercial (with real footage) → User material first frame → `img2video`
...
- `frame_strategy`: First/last frame strategy, options: none / first_frame_only / first_and_last_frame
  - **Note**: Omni mode (`generation_mode: omni-video`) does not use this field, as Omni uses `reference_images` instead of first-frame control
...
2. **Rhythm Variation**: Avoid all shots having the same duration
3. **Shot Scale Variation**: Consecutive shots should have shot scale differences
4. **Transition Selection**: Choose appropriate transitions based on emotion
5. **Single Action Principle**: Maximum 1 action within the same shot
6. **Spatial Invariance Principle**: No spatial environment changes within a shot
7. **Specific Description Principle**: No abstract action descriptions, use specific actions instead
...
- **Use the same model within the same project**

### Project Type Determination

| User Intent Keywords | Project Type |
|---------------------|--------------|
| "short drama", "storyline", "story" | Fiction/Short Drama |
| "vlog", "travel record", "life record" | Vlog/Documentary |
| "commercial", "promo video", "product showcase" | Commercial/Promo Video |
| "MV", "music video" | MV Short |

### Auto-Selection Decision Tree

**Fiction/Short Drama, MV Shorts**:
```
Fiction content → All shots require storyboard frame generation first
                 ├── Priority → Seedance (smart switching + multi-reference images)
                 │             └── image_urls: [storyboard frame, character refs...]
                 │             └── Time-segment prompt auto-triggers multi-shot
                 │
                 ├── Fallback 1 → Kling-3.0-Omni (reference2video)
                 │               └── image_list: [storyboard frame, character refs]
                 │
                 └── Fallback 2 → Kling-3.0 or Vidu Q3 Pro (img2video)
                                └── --image: storyboard frame as first frame
```

**Vlog/Documentary, Commercial/Promo Video (with real footage)**:
```
Real footage → Need first-frame control
...
|--------------|-----------------|-----------------|---------|-------------|
| Fiction/Short Drama | With/Without character refs | `seedance-video` | `seedance` | **Priority**: Smart switching + multi-reference |
...
| MV Short | With/Without character refs | `seedance-video` | `seedance` | **Priority**: Music-driven + smart switching |
| MV Short | Seedance unavailable | `omni-video` | `kling-omni` | Fallback: Mandatory storyboard frame |
| Vlog/Documentary | User real footage | `img2video` | `kling` or `vidu` | User material first-frame control |
| Commercial/Promo | With real footage | `img2video` | `kling` or `vidu` | Product/company material first frame |
| Commercial/Promo | Without real footage | `seedance-video` | `seedance` | **Priority**: Smart switching, long shots |
| Commercial/Promo | Without real footage + Seedance unavailable | `omni-video` | `kling-omni` | Fallback: Pure fiction showcase |

### Model and Generation Path Support

| Model | seedance-video | reference2video | img2video | text2video |
|-------|----------------|-----------------|-----------|------------|
| **Seedance** | ✅ Supported | ✅ Supported (image_urls) | ❌ No first-frame control | ✅ Supported |
...
**Key Points**:
- **Seedance does not support precise first-frame control**, storyboard frames serve as references not first frames
...
|---|--------------|------------------|
| `none` | No first/last frame needed | Direct text-to-video API call |
...
When a shot contains dialogue, **it must be fully described in video_prompt**: character (with appearance), dialogue content (in quotes), expression/emotion, voice quality and speech rate.

```json
{
  "shot_id": "scene1_shot5",
  "video_prompt": "Xiaomei (25-year-old Asian woman, long straight black hair) looks up at the server, smiling gently and saying: 'It's really quiet here, I like it.' Voice crisp and pleasant, speech rate moderate to slow. Maintain vertical 9:16 composition.",
  "dialogue": {
    "speaker": "Xiaomei",
    "content": "It's really quiet here, I like it.",
    "emotion": "Gentle, pleased",
...
```json
{
  "narration_config": {
    "enabled": true,
    "voice_style": "Gentle female voice, moderate to slow speech rate, full of emotion"
  },
  "narration_segments": [
    {
      "segment_id": "narr_1",
      ...
      "text": "This is a peaceful afternoon, sunlight streaming through floor-to-ceiling windows into the coffee shop..."
    },
    {
      "segment_id": "narr_2",
      "time_range": "8-11s",
      "target_shot": "scene1_shot3",
      "text": "She sits by the window, gazing at the scenery outside, her thoughts drifting far away..."
    }
  ]
}
```
...
| `time_range` | string | Overall timeline position (format: `0-3s`, `8-11s`, calculated from video start) |
...
2. **Segment Length**: Each narration segment should be speakable within 2-5 seconds (about 30-50 characters)
3. **Avoid Conflicts**: Narration time ranges should not overlap with character dialogue (sync sound)
4. **Shot Correspondence**: Each segment must correspond to a target_shot
5. **Unified voice_style**: Use the same narration style within one video
...
```json
{
  "text": "This is a peaceful afternoon, sunlight streaming through floor-to-ceiling windows into the coffee shop, she sits by the window, gazing at the scenery outside, her thoughts drifting far away, recalling that special summer."
}
```

**Do this** (segmented by shot):
```json
{
  "narration_segments": [
    {"segment_id": "narr_1", "time_range": "0-3s", "target_shot": "scene1_shot1", "text": "This is a peaceful afternoon, sunlight streaming through floor-to-ceiling windows into the coffee shop..."},
    {"segment_id": "narr_2", "time_range": "8-11s", "target_shot": "scene1_shot3", "text": "She sits by the window, gazing at the scenery outside..."},
    {"segment_id": "narr_3", "time_range": "15-18s", "target_shot": "scene2_shot1", "text": "Her thoughts drift far away, recalling that special summer..."}
  ]
}
```
...
After Phase 2 confirms narration needs, `creative.json` records:
```json
{
  "narration": {
    "type": "ai_generated",
...
- Segment `creative.narration.full_text` by shot timing points
- Write `creative.narration.voice_style` into `narration_config.voice_style`
...
| Field | Type | Description |
|-------|------|-------------|
| `enabled` | boolean | Whether to generate audio (includes ambient sound + dialogue), default true |
...
`dialogue` field usage: TTS generation, subtitle extraction, user quick review.

**TTS narration only for**: Opening/closing narration, scene descriptions not requiring character on-screen, emotional enhancement narration. **Shots that can record sync sound should not use TTS!**
  ...
  "scenes": [
    {
      ...
      "duration": 18,
      "narrative_goal": "Show the female protagonist's daily life in the coffee shop",
      ...
      "shots": [
        {
          ...
          "description": "Coffee shop panoramic view",
          "generation_mode": "text2video",
          "generation_backend": "kling",
          "video_prompt": "Cozy city coffee shop interior panoramic view, afternoon sunlight streaming through floor-to-ceiling windows, camera slowly pushing in. Maintain vertical 9:16 composition. No background music. Natural ambient sound only.",
          "frame_strategy": "none",
          "multi_shot": false,
          "dialogue": null,
          "transition": "fade_in",
...
- Note: This field is placed at storyboard root level, not repeated within shots
...
```json
{
  ...
  "video_prompt": "Xiaomei (<<<image_1>>>) wearing headphones, fully focused in front of a racing simulator. Vertical 9:16 composition.",
...
**Note**: Omni mode uses `reference_images` as reference images, not `frame_strategy` first-frame control. `frame_strategy` should be set to `none`. If storyboard frame generation is needed, use `image_prompt` to separately record the storyboard frame prompt.
...
- Standard structure fields (`shot_id`, `duration`, `generation_mode`, `generation_backend`, etc.) are still retained
- Three-layer structure expands `image_prompt` and `video_prompt` into more detailed structured fields
- `character_image_mapping` is always placed at **Storyboard global level**, not repeated within shots
...
| `reference_images` | Auto-added after `frame_generation` generation | Storyboard frame output + character reference images |
| `frame_strategy` | Always `"none"` | Omni doesn't use first-frame control |

### Design Philosophy

**Storyboard Frame**: Not just first-frame control, but also controls overall visuals (scene, style, lighting, atmosphere, color, makeup/styling)

**Video Generation**: References storyboard frame composition, overlays motion and character references
...
```json
{
  ...
  "storyboard": {
    "chinese_description": "Continuous action and dialogue (approx 7s) wide shot. Chuyue backs away flustered to outside the door...",
...
**Note**: `character_image_mapping` is always placed at **Storyboard global level**, not repeated within shots. V3-Omni structure uses `frame_generation.character_refs` to reference characters.
...
- `motion_overall`: Overall motion description
- `motion_segments`: Segmented motion (with timeline)
...
Both Kling and Kling Omni support multi-shot continuous shooting.
...
- **customize** (recommended): Precise control over each shot's content and duration

### Multi-Shot Rules

- Total duration 3-15s, each shot at least 1s
- Sum of all shot durations = total video duration

| Scene | Recommended Mode |
|-------|------------------|
...
**3. Prompt Standards**
- All video_prompt include aspect ratio information
...
## Display to User for Confirmation (Mandatory Step)

**Must wait for user's explicit confirmation before entering Phase 4!**

When confirming, display for each shot: scene info, generation mode, backend, video_prompt, image_prompt (if any), dialogue, transition, duration.
...
During Phase 3 storyboard generation, it's recommended to reserve a fallback plan for each shot to avoid temporarily writing `image_prompt`.
...
1. Read the shot's `fallback_plan` field
...
4. Execute the fallback generation flow
...
// Before fallback
{
  ...
  "generation_mode": "omni-video",
  "generation_backend": "kling-omni",
  "video_prompt": "Xiaomei (<<<image_1>>>) by the coffee shop window...",
  "reference_images": ["materials/personas/xiaomei_ref.jpg"],
  "frame_strategy": "none",
  "fallback_plan": {
    "mode": "img2video",
...
// After fallback (fallback_plan fields copied)
{
  ...
  "generation_backend": "kling",
  "video_prompt": "Xiaomei by the coffee shop window...",  // Removed <<<image_1>>> reference
  "reference_images": [],
  "frame_strategy": "first_frame_only",
  "image_prompt": "Cinematic start frame. 25-year-old Asian woman...",
...
### Scenarios Not Needing fallback_plan

- Pure `text2video` shots (no reference images needed, fallback meaningless)
- User explicitly states not accepting fallback
...
**Core Feature**: Time-segmented prompt auto-triggers multi-shot, execution phase merges multiple shots from the same scene into one API call.

**✅ Duration Support: Any integer from 4-15s**

During **Phase 3 storyboard design phase**, when selecting Seedance backend, scene total duration can be **any integer from 4 to 15 seconds** (e.g., 6s, 8s, 12s, etc., no longer limited to 5/10/15).
...
**Shot structure remains unchanged**: Seedance only merges during execution phase, doesn't change storyboard's `scenes → shots` structure.

| Model | Execution Method |
|-------|------------------|
| Kling/Vidu/Omni | Each shot calls API separately |
| **Seedance** | Multiple shots from same scene merged into one API call (time-segmented prompt) |

### Scene → Video Segment Division Rules

| Scene Duration | Number of Shots | Video Segment Planning |
|----------------|-----------------|------------------------|
| ≤15s | Any number (e.g., 3-5 shots) | **Single video segment**, time segmentation covers all shots |
| 16-30s | Many (e.g., 6-10 shots) | **2-3 video segments**, segmented coverage (e.g., 15s + 15s) |
| >30s | Many (e.g., >10 shots) | **3+ video segments**, segmented coverage |
...
```json
{
  ...
  "shots": [
    ...
    {"shot_id": "scene1_shot4", "duration": 5, "description": "Raise cup showcase"}
  ]
}
```
...
@image1 (character reference), [viewpoint setting] [subject/style];

Overall: [overall camera motion description]

Segmented motion ({duration}s):
0-Xs: [scene] + [action] + [camera movement] + [rhythm] + [sound/dialogue];
X-Xs: [scene switch] + [scene] + [action] + [camera movement] + [rhythm] + [sound/dialogue];
...

Maintain {aspect_ratio} composition, don't break frame ratio
{BGM_constraint}
```
...
| Maximum 720p | Use Kling/Vidu when 1080p needed |
