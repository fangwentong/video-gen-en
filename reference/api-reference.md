...
python ~/.claude/skills/video-gen/video_gen_tools.py video --image <first_frame> --prompt <description> --output <output>

# Kling first/last frame control
python ~/.claude/skills/video-gen/video_gen_tools.py video --image <first_frame> --tail-image <last_frame> --prompt "action description" --backend kling --duration 5
...
python ~/.claude/skills/video-gen/video_gen_tools.py video --backend kling-omni --prompt "story" --image-list <ref_image_1> <ref_image_2> --multi-shot --shot-type customize --multi-prompt '[{"index":1,"prompt":"<<<image_1>>> shot 1","duration":"3"}]' --duration 7

# Auto backend selection (providing --image-list auto-uses kling-omni, providing --tail-image auto-uses kling)
python ~/.claude/skills/video-gen/video_gen_tools.py video --prompt "<<<image_1>>> at the arena" --image-list ref.jpg --output out.mp4

# Seedance 2 auto-assembly mode (recommended: auto-calculate time segments from storyboard, assemble prompt, arrange image_urls)
...
python ~/.claude/skills/video-gen/video_gen_tools.py video --backend seedance --prompt "time segment prompt..." --image-list frame.png ref.jpg --duration 8 --output out.mp4
...
# Veo3 text-to-video (Google Veo3, supports 4/6/8s only)
...
| `--audio` | kling, kling-omni | Enable audio-video sync generation |
...
| `--prompt` | Manual mode: directly specify time segment prompt (fallback use) |
| `--image-list` | Manual mode: image list (storyboard images first, character reference images after) |
| `--duration` | Duration: **4-15s (any integer)** |
| `--mode` | Generation mode: `text_to_video` (default) / `first_last_frames` (first+last frames) / `omni_reference` (multi-reference) |
...
| `--duration` | Duration: supports 4/6/8 seconds only (auto-aligns to nearest value) |
...
|------|------|---------|
| COMPASS_API_KEY | Gemini image generation + Gemini TTS + **Veo3 video** | When generating images/TTS/Veo3 video |
| FAL_API_KEY | Gemini image generation + Kling-Omni video (fal.ai proxy) | When generating images/video (backup) |
...
| VISION_BASE_URL | Vision model API URL | When customizing vision model |
